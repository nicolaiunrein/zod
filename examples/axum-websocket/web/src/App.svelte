...
/* let messages = [{user:"self", content: "What is here the question?"}, {user: "max", content: "re: abc"}] */
let color = "#FF00FF";
let name = "John Doe"
let current_msg = ""
...
<div class="p:2 sm:p-6 justify-between flex flex-col bg-gray-200 my-32 rounded-xl shadow-xl w-full flex-grow">
   <div class="flex sm:items-center justify-between py-3 border-b-2 border-gray-300">
      <div class="relative flex items-center space-x-4">
         <div class="relative">
            <div class="rounded-full overflow-hidden grid items-center justify-center w-10 h-10 sm:w-16 h-10 sm:h-16 border-2">
                <input type="color" class="h-full border-none outline-none cursor-pointer" bind:value={color}/>
                <div class="absolute w-full h-full p-4 pointer-events-none">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-full h-full mx-auto text-white pointer-events-none" >
         ...
         <div class="flex flex-col leading-tight">
            ...
            <input class="bg-transparent text-gray-700 outline-none" placeholder="Name" bind:value={name}/>
            </div>
         </div>
      </div>
   </div>
   <div id="messages" class="flex flex-col space-y-4 pt-10 overflow-y-auto scrollbar-thumb-blue scrollbar-thumb-rounded scrollbar-track-blue-lighter scrollbar-w-2 scrolling-touch flex-grow">
   {#each history as msg}
