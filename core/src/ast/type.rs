use super::{Delimited, FormatResolvedTs, FormatResolvedZod, FormatTypescript, FormatZod, Generic};
...
impl FormatResolvedZod for TypeDef {
    fn fmt_resolved_zod(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        if !self.generics.is_empty() {
            ...
            Delimited(tys.as_slice(), ", ").fmt_resolved_zod(f)?;
            f.write_str(")")?;
        }
...
impl FormatResolvedTs for TypeDef {
